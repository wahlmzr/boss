...
class BossspiderPipeline(object):
    #打开爬虫时，连接数据库connect('database_name')
    def open_spider(self,spider):
        self.con = pymysql.connect('192.168.20.107','root','1987','boss', charset='utf8')
        self.cu = self.con.cursor()


    def process_item(self, item, spider):
        #sql插入语句，将爬取的数据插入到数据库
        insert_sql = "insert into zhipin (post,salary,company,area,exp,edu,industry,detail ) VALUES ('{}','{}','{}','{}','{}','{}','{}')"\
            .format(item['post'],item['salary'],item['company'],item['area'],item['exp'],item['edu'],item['industry'] , item['detail_page'])
        #执行插入语句
        self.cu.execute(insert_sql)
        #数据提交（数据插入或更新）需要commit
        self.con.commit()
